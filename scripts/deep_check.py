...
def head_size(url: str) -> tuple[int, int]:
    ...
    try:
        req = urllib.request.Request(url, method="HEAD",
                                     headers={"User-Agent": "public-transit-api/1.0"})
...
def routable_smoke(fid: str) -> tuple[bool, str]:
    ...
    pts = db.execute("""
        ...
        (trip_id, lo, hi)).fetchall()
    db.close()
    if len(pts) < 2:
        return False, "trip stops unresolved"
    (alat, alon), (blat, blon) = pts[0], pts[-1]
    routing._stops.cache_clear(); routing._footpaths.cache_clear()
...
def check(feed: dict) -> dict:
    ...
    rec = {"id": fid, "country": feed.get("country"), "city": feed.get("city_region"),
           "reachable": False, "ingested": False, "routable": False, "note": ""}
    ...
    rec["reachable"] = True
    try:
        proc = subprocess.run([sys.executable, "-m", "app.ingest", fid],
                              capture_output=True, text=True,
                              timeout=INGEST_TIMEOUT, cwd=ROOT)
...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as pool:
        for i, rec in enumerate(pool.map(check, sample), 1):
            ...
            stage = ("routable" if rec["routable"] else
                     "ingested" if rec["ingested"] else
                     "reachable" if rec["reachable"] else "dead")
            print(f"[{i:>3}/{len(sample)}] {stage:<9} {rec['id']:<10} "
                  f"{(rec['city'] or '')[:34]:<34} {rec['note'][:60]}", file=sys.stderr)
    ...
    json.dump({"date": date, "sample": N, "reachable": r, "ingested": g,
               "routable": t, "results": results},
